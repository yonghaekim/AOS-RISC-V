 ...
 */
//yh+begin
class RobWYFYConfig(implicit p: Parameters) extends BoomBundle()(p)
{
  val enableWYFY          = Bool()
  val num_pacma           = UInt(xLen.W)
  val num_xpacm           = UInt(xLen.W)
}
//yh+end
class RobIo(
  ...
  )(implicit p: Parameters)  extends BoomBundle
{
  ...
  //yh+begin
  val wyfy_config = Input(new RobWYFYConfig)
  val num_pacma   = Output(UInt(xLen.W))
  val num_xpacm   = Output(UInt(xLen.W))
  //yh+end
}
  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val rob_head_is_pacma = Wire(Vec(coreWidth, Bool()))
  val rob_head_is_xpacm = Wire(Vec(coreWidth, Bool()))

  enableWYFY  := io.wyfy_config.enableWYFY
  initWYFY    := io.wyfy_config.enableWYFY & !enableWYFY

  val num_pacma = RegInit(0.U(xLen.W))
  val num_xpacm = RegInit(0.U(xLen.W))

  io.num_pacma := num_pacma
  io.num_xpacm := num_xpacm

  var temp_num_pacma = num_pacma
  var temp_num_xpacm = num_xpacm
  //yh+end
  ...
  for (w <- 0 until coreWidth) {
    ...
    when (io.enq_valids(w)) {
      ...
      ////rob_bsy(rob_tail)       := !(io.enq_uops(w).is_fence ||
      ////                             io.enq_uops(w).is_fencei ||
      ////                             io.enq_uops(w).uses_bdq)
      //rob_bsy(rob_tail)       := !(io.enq_uops(w).is_fence ||
      //                             io.enq_uops(w).is_fencei ||
      //                             io.enq_uops(w).uopc === uopBNDSTR ||
      //                             io.enq_uops(w).uopc === uopBNDCLR)
      ////yh+end
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
    ...
    //yh+begin
    rob_head_is_pacma(w) := (rob_uop(rob_head).uopc === uopPACMA)
    rob_head_is_xpacm(w) := (rob_uop(rob_head).uopc === uopXPACM)


    temp_num_pacma = Mux(initWYFY, io.wyfy_config.num_pacma,
                      Mux(will_commit(w) && rob_head_is_pacma(w),
                          temp_num_pacma + 1.U, temp_num_pacma))

    temp_num_xpacm = Mux(initWYFY, io.wyfy_config.num_xpacm,
                      Mux(will_commit(w) && rob_head_is_xpacm(w),
                          temp_num_xpacm + 1.U, temp_num_xpacm))

    when (initWYFY)
    {
      printf("YH+ initWYFY num_pacma: %x num_xpacm: %x\n",
              io.wyfy_config.num_pacma, io.wyfy_config.num_xpacm)
    }
      .elsewhen (will_commit(w) && rob_head_is_pacma(w))
    {
      printf("YH+ Increment num_pacma: %x\n", temp_num_pacma)
    }
      .elsewhen (will_commit(w) && rob_head_is_xpacm(w))
    {
      printf("YH+ Increment num_xpacm: %x\n", temp_num_xpacm)
    }
    //yh+end
	...
	//yh+begin
  num_pacma := temp_num_pacma
  num_xpacm := temp_num_xpacm
	//yh+end
