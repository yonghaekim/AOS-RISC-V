  ...
  with HasBoomCoreParameters
{
  ...
  val s1_valid = widthMap(w =>
                 RegNext(s0_valid(w)                                     &&
                         ...
                         !(io.lsu.exception && (s0_req(w).uop.uses_ldq || s0_req(w).uop.is_bndload))   && //yh+
                         //yh-!(s2_store_failed && io.lsu.req.fire && s0_req(w).uop.uses_stq),
                         !(s2_store_failed && io.lsu.req.fire && (s0_req(w).uop.uses_stq || s0_req(w).uop.is_bndstore)), //yh+
                         init=false.B))
  for (w <- 0 until memWidth)
  ...
  val s2_valid = widthMap(w =>
                  RegNext(s1_valid(w) &&
                         ...
                         !(io.lsu.exception && (s1_req(w).uop.uses_ldq || s1_req(w).uop.is_bndload)) && //yh+
                         //yh-!(s2_store_failed && (s1_type === t_lsu) && s1_req(w).uop.uses_stq)))
                         !(s2_store_failed && (s1_type === t_lsu) && (s1_req(w).uop.uses_stq || s1_req(w).uop.is_bndstore)))) //yh+
  for (w <- 0 until memWidth)
  ...
  //yh-s2_store_failed := s2_valid(0) && s2_nack(0) && s2_send_nack(0) && s2_req(0).uop.uses_stq
  s2_store_failed := s2_valid(0) && s2_nack(0) && s2_send_nack(0) && (s2_req(0).uop.uses_stq || s2_req(0).uop.is_bndstore) //yh+
  ...
  for (w <- 0 until memWidth) {
    mshrs.io.req(w).valid := s2_valid(w)          &&
                            ...
                            !(io.lsu.exception && (s2_req(w).uop.uses_ldq || s2_req(w).uop.is_bndload))   && //yh+
  ...
  enableWYFY          := io.lsu.enableWYFY
  initWYFY            := io.lsu.enableWYFY & !enableWYFY

  val num_cache_hit   = RegInit(0.U(xLen.W))
  val num_cache_miss  = RegInit(0.U(xLen.W))

  var temp_num_cache_hit = num_cache_hit
  var temp_num_cache_miss = num_cache_miss
  ...
  for (w <- 0 until memWidth) {
    val is_cache_hit = s2_valid(w)          &&
                        ...
                        !(io.lsu.exception && s2_req(w).uop.mem_cmd === rocket.M_XRD)
    ...
    temp_num_cache_hit = Mux(initWYFY, io.lsu.num_cache_hit_in,
                              Mux(is_cache_hit, temp_num_cache_hit + 1.U,
                              temp_num_cache_hit))

    temp_num_cache_miss = Mux(initWYFY, io.lsu.num_cache_miss_in,
                              Mux(is_cache_miss, temp_num_cache_miss + 1.U,
                              temp_num_cache_miss))
  }

  num_cache_hit := temp_num_cache_hit
  num_cache_miss := temp_num_cache_miss

  io.lsu.num_cache_hit_out := num_cache_hit
  io.lsu.num_cache_miss_out := num_cache_miss
  //yh+end
  ...
  for (w <- 0 until memWidth) {
    io.lsu.resp(w).valid := resp(w).valid &&
                            ...
                            !(io.lsu.exception && (resp(w).bits.uop.uses_ldq || resp(w).bits.uop.is_bndload)) && //yh+
