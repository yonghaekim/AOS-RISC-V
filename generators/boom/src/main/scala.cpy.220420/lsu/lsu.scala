  ...
  with rocket.HasL1HellaCacheParameters
{
    ...
    .elsewhen (io.core.brupdate.b2.mispredict)
  {
	...
	// Here, addr is base address
	val exe_bdq_vaddr = widthMap(w => exe_req(w).bits.addr)
	// Here, data is size
	val exe_bdq_data = widthMap(w => exe_req(w).bits.data)
  ...
  for (w <- 0 until memWidth)
	{
		when (exe_bdq_val(w))
		{
			...
			bdq(bidx).bits.data         := Cat(exe_bdq_data(w)(31,0), exe_bdq_vaddr(w)(31,0))
			bdq(bidx).bits.state        := b_occChk // Go to b_occChk

			printf("YH+ [%d] Exe bdq(%d) exe_req vaddr: %x PAC: %d size: %d\n",
			  io.core.tsc_reg, bidx, exe_bdq_vaddr(w), exe_bdq_vaddr(w)(xLen-1,xLen-pacWidth), exe_bdq_data(w))
		}
	}
  ...
  val exe_mcq_val = widthMap(w => exe_req(w).valid
                                    && (exe_req(w).bits.uop.ctrl.is_sta
                                        || exe_req(w).bits.uop.ctrl.is_load)
																		&& !exe_req(w).bits.uop.is_fence
																		&& !exe_req(w).bits.uop.is_fencei)
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  val stq_incoming_e   = widthMap(i => stq(stq_incoming_idx(i)))
	...
	//yh+begin
	val bdq_incoming_idx = widthMap(i => exe_req(i).bits.uop.bdq_idx)
  ...
  val can_fire_bnd_incoming  = widthMap(w => exe_req(w).valid && exe_req(w).bits.uop.uses_bdq
																							&& exe_req(w).bits.uop.uopc =/= uopBNDSRCH) // BNDSRCH will write back later
	//yh+end
  ...
  for (w <- 0 until memWidth) {
      ...
      .elsewhen (!dmem_resp_fired(w) && wb_forward_valid(w))
    {
      ...
      when (data_ready && live) {
		...
		//yh+begin
		when ((w == memWidth-1).B && !dmem_resp_fired(memWidth-1) &&
					!wb_forward_valid(memWidth-1) && bdq_resp_val)
		{
      ...
      val bnd_uop     = bdq_bnd_e.bits.uop
      val live        = !IsKilledByBranch(io.core.brupdate, bnd_uop)

      io.core.exe(w).iresp.valid := live
      io.core.exe(w).iresp.bits.uop  := bnd_uop
      io.core.exe(w).iresp.bits.data := Mux(bdq_bnd_e.bits.state === b_bndResp, 1.U, 0.U)
  ...
  //yh+begin
  val mcq_load_resp_val = widthMap(w =>
                                  Mux(io.dmem.resp(w).valid,
		                                  io.dmem.resp(w).bits.uop.uses_mcq, false.B))

  val mcq_load_resp_idx = widthMap(w =>
                                  Mux(io.dmem.resp(w).valid && io.dmem.resp(w).bits.uop.uses_mcq,
                                      io.dmem.resp(w).bits.uop.mcq_idx, 0.U))
  val mcq_load_resp_data = widthMap(w =>
                                  Mux(io.dmem.resp(w).valid && io.dmem.resp(w).bits.uop.uses_mcq,
                                      io.dmem.resp(w).bits.data, 0.U))

  val bdq_load_resp_val = widthMap(w =>
                                  Mux(io.dmem.resp(w).valid,
		                                  (io.dmem.resp(w).bits.uop.uses_bdq &
																			io.dmem.resp(w).bits.uop.is_bndload), false.B))

  val bdq_load_resp_idx = widthMap(w =>
                                  Mux(io.dmem.resp(w).valid && io.dmem.resp(w).bits.uop.uses_bdq,
                                      io.dmem.resp(w).bits.uop.bdq_idx, 0.U))

  val bdq_load_resp_data = widthMap(w =>
                                  Mux(io.dmem.resp(w).valid && io.dmem.resp(w).bits.uop.uses_bdq,
                                      io.dmem.resp(w).bits.data, 0.U))
  val bdq_load_resp_bndstr = widthMap(w =>
                                  Mux(io.dmem.resp(w).valid,
                                      (io.dmem.resp(w).bits.uop.uopc === uopBNDSTR), false.B))

  val bdq_load_resp_bndclr = widthMap(w =>
                                  Mux(io.dmem.resp(w).valid,
                                      (io.dmem.resp(w).bits.uop.uopc === uopBNDCLR), false.B))

  val bdq_load_resp_bndsrch = widthMap(w =>
                                  Mux(io.dmem.resp(w).valid,
                                      (io.dmem.resp(w).bits.uop.uopc === uopBNDSRCH), false.B))
