...
// DOC include start: BOOM Parameters
  ...
  //yh+begin
  numMcqEntries: Int = 16,
  numBdqEntries: Int = 16,
  hbtAssocWidth: Int = 8,
  pacWidth:      Int = 16,
  //yh+end
...
class BoomCustomCSRs(implicit p: Parameters) extends freechips.rocketchip.tile.CustomCSRs
  ...
  override def numPacmaCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    val mask = BigInt(0x7FFFFFFFFFFFFFFFL)
    val init = BigInt(0x0)
    Some(CustomCSR(numPacmaCSRId, mask, Some(init)))
  }

  override def numXpacmCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    val mask = BigInt(0x7FFFFFFFFFFFFFFFL)
    val init = BigInt(0x0)
    Some(CustomCSR(numXpacmCSRId, mask, Some(init)))
  }

  override def numBndStrFailCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    val mask = BigInt(0x7FFFFFFFFFFFFFFFL)
    val init = BigInt(0x0)
    Some(CustomCSR(numBndStrFailCSRId, mask, Some(init)))
  }

  override def numBndClrFailCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    val mask = BigInt(0x7FFFFFFFFFFFFFFFL)
    val init = BigInt(0x0)
    Some(CustomCSR(numBndClrFailCSRId, mask, Some(init)))
  }

  override def numBndChkFailCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    val mask = BigInt(0x7FFFFFFFFFFFFFFFL)
    val init = BigInt(0x0)
    Some(CustomCSR(numBndChkFailCSRId, mask, Some(init)))
  }
...
trait HasBoomCoreParameters extends freechips.rocketchip.tile.HasCoreParameters
{
  ...
  val numBdqEntries = boomParams.numBdqEntries       // number of BDQ entries
  val hbtAssocWidth = boomParams.hbtAssocWidth          // how many rows there are in the HBT
  val pacWidth      = boomParams.pacWidth            // number of BDQ entries
  //yh+end
