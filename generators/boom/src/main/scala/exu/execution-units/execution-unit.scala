  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val io = IO(new Bundle {
    ...
    val bdq_io = if (hasAlu) Flipped(new boom.lsu.BDQExeIO) else null //yh+
  ...
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  ...
  if (hasAlu) {
    ...
    //yh+begin
    io.bdq_io.req.valid := io.req.valid && io.req.bits.uop.fu_code_is(FU_ALU) &&
                            io.req.bits.uop.uses_bdq &&
                            !IsKilledByBranch(io.brupdate, io.req.bits.uop)
    io.bdq_io.req.bits.uop := io.req.bits.uop
    io.bdq_io.req.bits.addr := io.req.bits.rs1_data
    io.bdq_io.req.bits.data := io.req.bits.rs2_data
    //yh+end
