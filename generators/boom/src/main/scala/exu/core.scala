  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  //yh+begin
  val alu_units = exe_units.alu_units
  for (i <- 0 until intWidth) {
    alu_units(i).io.bdq_io <> io.lsu.bdq_exe(i) //yh+
  }
  //yh+end
...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  val dis_hazards = (0 until coreWidth).map(w =>
                      ...
                      //yh+begin
                      || io.lsu.mcq_full(w) && dis_uops(w).uses_mcq
...
//   "CSRFile is being written to spuriously.")
  ...
  io.lsu.wyfy_config.num_bndchk_fail    := custom_csrs.num_bndchk_fail
  ...
  csr.io.wyfyConfig         := Cat(custom_csrs.enableWYFY)
  ...
  csr.io.num_bndchk_fail    := io.lsu.num_bndchk_fail
