...
class LSUCoreIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val num_bndchk_fail   = Output(UInt(xLen.W))
...
class LSUWYFYConfig(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val num_bndchk_fail     = UInt(xLen.W)
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val num_bndchk_fail     = RegInit(0.U(xLen.W))
  ...
  io.core.num_bndchk_fail   := num_bndchk_fail
  ...
  val bdq_load_idx = RegNext(AgePriorityEncoder((0 until numBdqEntries).map(i => {
    val e = bdq(i).bits
    e.addr.valid && !e.executed && e.state === b_occChk
  }), bdq_head))
  val bdq_load_e = bdq(bdq_load_idx)

  // Mask gen.
  val pac_match_bdq = Wire(Vec(numBdqEntries, Bool()))
  val bdq_load_pac = bdq_load_e.bits.addr.bits(xLen-1,xLen-pacWidth)

  for (i <- 0 until numBdqEntries) {
    val temp_pac = bdq(i).bits.addr.bits(xLen-1,xLen-pacWidth)

    pac_match_bdq(i) := bdq_load_e.valid &&
                  IsOlder(i.U, bdq_load_idx, bdq_head) &&
                  (bdq(i).bits.state =/= b_done) &&
                  (!bdq(i).bits.addr.valid ||
                    temp_pac === bdq_load_pac)

    when (bdq_load_e.valid && IsOlder(i.U, bdq_load_idx, bdq_head))
    {
      printf("YH+ [%d] pac_match_bdq bdq(%d): %d <-> bdq(%d): %d\n",
              io.core.tsc_reg, i.U, temp_pac, bdq_load_idx, bdq_load_pac)
    }
  }

  val bdq_load_val = (bdq_load_e.valid                    	&&
											bdq_load_e.bits.addr.valid						&&
                      !bdq_load_e.bits.executed						&&
                      !pac_match_bdq.reduce(_|_)          &&
                      bdq_load_e.bits.state === b_occChk)

  when (bdq_load_e.valid											&&
					bdq_load_e.bits.addr.valid					&&
          bdq_load_e.bits.state === b_occChk &&
          !bdq_load_e.bits.executed)
  {
    when (pac_match_bdq.reduce(_|_))
    {
      printf("YH+ [%d] pac_match_bdq bdq(%d) prevented bounds load!\n",
              io.core.tsc_reg, bdq_load_idx)
    }
  }

	// TODO: BDQ store execute head
  val bdq_store_idx = RegNext(AgePriorityEncoder((0 until numBdqEntries).map(i => {
    val e = bdq(i).bits
    e.addr.valid && !e.executed && e.state === b_bndStr
  }), bdq_head))
  val bdq_store_e = bdq(bdq_store_idx)

  val bdq_store_val = (bdq_store_e.valid                 	&&
											bdq_store_e.bits.addr.valid					&&
                      !bdq_store_e.bits.executed					&&
                      bdq_store_idx === bdq_head          &&
                      bdq_store_e.bits.state === b_bndStr &&
                      bdq_store_e.bits.committed)

	when (bdq_load_val)
	{
	  printf("YH+ [%d] bdq_load_idx: %d bdq_load_val: %d\n",
						io.core.tsc_reg, bdq_load_idx, bdq_load_val)
	}

  val bdq_resp_val = Wire(Vec(memWidth, Bool()))
	bdq_resp_val := widthMap(w => bdq_load_e.valid         &&
											bdq_load_e.bits.addr.valid					&&
											bdq_load_e.bits.uop.uopc === uopBNDSRCH &&
                      bdq_load_idx === bdq_head					&&
											(bdq_load_e.bits.state === b_bndResp ||
											bdq_load_e.bits.state === b_fail) &&
											(w == 0).B)

	//val bdq_resp_val = (bdq_load_e.valid                 	&&
	//										bdq_load_e.bits.addr.valid					&&
	//										bdq_load_e.bits.uop.uopc === uopBNDSRCH &&
  //                    bdq_load_idx === bdq_head					&&
	//										(bdq_load_e.bits.state === b_bndResp ||
  ...
  // Mask gen.
  val pac_match_mcq = Wire(Vec(numBdqEntries, Bool()))
  val mcq_load_pac = mcq_load_e.bits.addr(xLen-1,xLen-pacWidth)
  val youngest_bdq_idx = mcq_load_e.bits.youngest_bdq_idx

  for (i <- 0 until numBdqEntries) {
    val temp_pac = bdq(i).bits.addr.bits(xLen-1,xLen-pacWidth)

    pac_match_mcq(i) := mcq_load_e.valid &&
                  bdq(i).valid &&
                  IsOlder(i.U, youngest_bdq_idx, bdq_head) &&
                  bdq(i).bits.state =/= b_done &&
                  bdq(i).bits.uop.uopc =/= uopBNDSRCH &&
                  (!bdq(i).bits.addr.valid ||
                    temp_pac === mcq_load_pac)

    when (mcq_load_e.valid && IsOlder(i.U, youngest_bdq_idx, bdq_head))
    {
      printf("YH+ [%d] pac_match_mcq bdq(%d): %d <-> mcq(%d): %d youngest_bdq_idx: %d\n",
              io.core.tsc_reg, i.U, temp_pac, mcq_load_idx, mcq_load_pac, youngest_bdq_idx)
    }
  }

  val mcq_load_val = (mcq_load_e.valid                        &&
                      mcq_load_e.bits.state === m_bndChk      &&
                      !pac_match_mcq.reduce(_|_)              &&
                      !mcq_load_e.bits.executed)

  when (mcq_load_e.valid &&
          (mcq_load_e.bits.state === m_bndChk) &&
          !mcq_load_e.bits.executed)
  {
    when (pac_match_mcq.reduce(_|_))
    {
      printf("YH+ [%d] pac_match_mcq mcq(%d) prevented bounds load!\n",
  ...
  val bnd_load_pac = Mux(bdq_load_val, bdq_load_e.bits.addr.bits(xLen-1,xLen-pacWidth),
											Mux(mcq_load_val, mcq_load_e.bits.addr(xLen-1,xLen-pacWidth), 0.U))
  val bnd_load_count = Mux(bdq_load_val, bdq_load_e.bits.count, 
												Mux(mcq_load_val, mcq_load_e.bits.count, 0.U))
  ...
  val bnd_load_uop = Mux(bdq_load_val, bdq_load_e.bits.uop,
											Mux(mcq_load_val, mcq_load_e.bits.uop, NullMicroOp))

  //val bnd_store_pac = 0.U
  val bnd_store_pac = bdq_store_e.bits.addr.bits(xLen-1,xLen-pacWidth)
  val bnd_store_count = bdq_store_e.bits.count
  //val bnd_store_paddr = (hbt_base_addr + Cat((bnd_store_pac << 2), (bdq_store_e.bits.count << 3))) // assume log2Ceil(hbt_num_way) == 2
  val bnd_store_paddr = (hbt_base_addr + (bnd_store_pac << 5) + (bnd_store_count << 3))
  //val bnd_store_paddr = (hbt_base_addr | (bnd_store_pac << 6) | (bdq_store_e.bits.count << 3))
  val bnd_store_uop = bdq_store_e.bits.uop
	val bnd_store_data = Mux(bdq_store_e.bits.uop.uopc === uopBNDCLR, 0.U, bdq_store_e.bits.data)

  val bnd_resp_pac = bdq_load_e.bits.addr.bits(xLen-1,xLen-pacWidth)
  val bnd_resp_count = bdq_load_e.bits.count
  val bnd_resp_paddr = (hbt_base_addr + (bnd_resp_pac << 5) + (bnd_resp_count << 3))
  val bnd_resp_data = Wire(UInt(xLen.W))
  bnd_resp_data := Mux(bdq_load_e.bits.state === b_bndResp, 1.U, 0.U)
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  for (w <- 0 until memWidth) {
      ...
      .elsewhen (will_fire_bnd_load(w))
    {
      ...
      mcq_load_e.bits.executed        	:= Mux(mcq_load_val & !bdq_load_val, dmem_req_fire(w),
																							mcq_load_e.bits.executed)
      bdq_load_e.bits.executed        	:= Mux(bdq_load_val, dmem_req_fire(w),
																							bdq_load_e.bits.executed)
      ...
      .elsewhen (will_fire_bnd_store(w))
    {
      ...
      when (dmem_req_fire(w) && will_fire_bnd_store(w) && bdq_store_val)
      {
        printf("YH+ [%d] Send bounds store bdq(%d) paddr: %x data: %x\n",
                io.core.tsc_reg, bdq_store_idx, bnd_store_paddr, bnd_store_data)
      }
    }
		//yh+end
  ...
  for (w <- 0 until memWidth) {
      ...
      .elsewhen (!dmem_resp_fired(w) && wb_forward_valid(w))
    {
      ...
      when (data_ready && live) {
									...
									bdq_resp_val(w))
		{
			printf("YH+ [%d] Found bdq_resp_val! bdq_resp_idx: %d bnd_resp_val: %d\n",
							io.core.tsc_reg, bdq_load_idx, Mux(bdq_load_e.bits.state === b_bndResp, 1.U, 0.U))

      val bdq_uop     = bdq_load_e.bits.uop
      ...
      when (live) {
  			bdq_load_e.bits.executed := true.B
  			bdq_load_e.bits.state := b_done
      }
		}
		//yh+end
  }
  ...
  for (w <- 0 until memWidth) {
    when (mcq_load_resp_val(w))
    {
	    ...
	    val mcq_uop = mcq(mcq_idx).bits.uop
			...
			assert(mcq(mcq_idx).valid && mcq(mcq_idx).bits.state === m_bndChk)

      printf("YH+ [%d] mcq_load_resp_val(%d) is true\n",
              io.core.tsc_reg, w.U)

      val mcq_lbnd = Wire(UInt(33.W))
			val mcq_ubnd = Wire(UInt(33.W))
      val c_bit = Wire(UInt(1.W))
			val mcq_addr = Wire(UInt(33.W))

			mcq_lbnd := mcq_load_resp_data(w)(31,0)
      mcq_ubnd := (mcq_load_resp_data(w)(63,32) + mcq_load_resp_data(w)(31,0))
			c_bit := (mcq_load_resp_data(w)(31) & !mcq(mcq_idx).bits.addr(31))
      mcq_addr := Cat(c_bit, mcq(mcq_idx).bits.addr(31,0))
    ...
    when (bdq_load_resp_val(w))
    {
	    ...
	    val bdq_data = bdq(bdq_idx).bits.data
			...
			assert(bdq(bdq_idx).valid && bdq(bdq_idx).bits.state === b_occChk)
  ...
  when (initWYFY)
  {
    ...
    num_bndchk_fail       := io.core.wyfy_config.num_bndchk_fail
  }
    .elsewhen (bdq_clear)
  {
  ...
  when (initWYFY)
  {
    ...
    .elsewhen (mcq_clear)
  {
		when (mcq(mcq_head).bits.state === m_fail)
		{
			num_bndchk_fail   := num_bndchk_fail + 1.U
		}
  }
