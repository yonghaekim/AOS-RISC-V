  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val bnd_store_paddr = (hbt_base_addr + (bnd_store_pac << 5) + (bdq_bnd_e.bits.count << 3))
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  for (w <- 0 until memWidth) {
    ...
    when (io.dmem.resp(w).valid)
    {
      ...
      //yh+begin
        .elsewhen (io.dmem.resp(w).bits.uop.uses_mcq)
      {
        val mcq_idx = io.dmem.resp(w).bits.uop.mcq_idx
				val bnd_data = io.dmem.resp(w).bits.data
				val mcq_lbnd = Cat(false.B, bnd_data(31,0))
				val mcq_ubnd = Cat(false.B, bnd_data(63,32) + bnd_data(31,0))
				val c_bit = (bnd_data(31) & !mcq(mcq_idx).bits.addr(31))
	      ...
	      val count = mcq(mcq_idx).bits.count
				...
				printf("YH+ [%d] Received Load RESP mcq(%d)\n",
        ...
        .elsewhen (io.dmem.resp(w).bits.uop.uses_bdq)
      {
        ...
        val bdq_resp_bndstr = (io.dmem.resp(w).bits.uop.uopc === uopBNDSTR)
        val bdq_resp_bndclr = (io.dmem.resp(w).bits.uop.uopc === uopBNDCLR)
        val bdq_resp_bndsrch = (io.dmem.resp(w).bits.uop.uopc === uopBNDSRCH)
				val bdq_data = bdq(bdq_idx).bits.data

				val occ_chk = Mux(bdq_resp_bndstr, io.dmem.resp(w).bits.data === 0.U, //TODO
														Mux(bdq_resp_bndsrch, io.dmem.resp(w).bits.data === bdq_data,
														io.dmem.resp(w).bits.data(31,0) === bdq_data(31,0)))
				val count = bdq(bdq_idx).bits.count

        //when (bdq(bdq_idx).bits.state === b_bndStr)
        when (!io.dmem.resp(w).bits.uop.is_bndload)
        {
          ...
          printf("YH+ [%d] Received Store RESP bdq(%d)\n",
                  io.core.tsc_reg, bdq_idx)
					...
					//.elsewhen (bdq(bdq_idx).bits.state === b_occChk)
					.otherwise
				{
					printf("YH+ [%d] Received Load RESP bdq(%d)\n",
                  io.core.tsc_reg, bdq_idx)

					when (occ_chk)
					{
						when (bdq_resp_bndsrch)
						{
							bdq(bdq_idx).bits.state           := b_done
							printf("YH+ [%d] Passed occupancy check (bndsrch)! bdq(%d) data: %x bnd: %x\n",
											io.core.tsc_reg, bdq_idx, bdq_data, io.dmem.resp(w).bits.data)

							io.core.exe(w).iresp.valid 				:= true.B
							io.core.exe(w).iresp.bits.uop  		:= bdq(bdq_idx).bits.uop
							io.core.exe(w).iresp.bits.data 		:= 1.U
						}
							.otherwise
						{
						...
						.elsewhen (count < (hbt_num_way - 1.U))
					{
						...
						when (bdq_resp_bndstr)
						{
							printf("YH+ [%d] Increase counter (bndstr) (%d->%d) bdq(%d) data: %x bnd: %x\n",
											io.core.tsc_reg, count, count+1.U, bdq_idx, bdq_data, io.dmem.resp(w).bits.data)
						}
							.elsewhen(bdq_resp_bndsrch)
						{
						...
						.otherwise
					{
						when(bdq_resp_bndsrch)
						{
							bdq(bdq_idx).bits.state           := b_done // bounds search fail is not an error
