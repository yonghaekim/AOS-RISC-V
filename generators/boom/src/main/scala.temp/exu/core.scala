...
//yh-import freechips.rocketchip.rocket.{Causes, PRV}
import freechips.rocketchip.rocket.{Causes, PRV, CSR} //yh+
...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  for (w <- 0 until coreWidth) {
    ...
    decode_units(w).io.enableWYFY      := custom_csrs.enableWYFY //yh+

    dec_uops(w) := decode_units(w).io.deq.uop

    //yh+begin
    when (decode_units(w).io.deq.uop.uses_bdq)
    {
      printf("YH+ [%d] Found bounds inst at decode stage\n", debug_tsc_reg)
    }

    when (decode_units(w).io.deq.uop.uopc === uopPACMA)
    {
      printf("YH+ Found uopPACMA at decode stage!\n")
    }
      .elsewhen (decode_units(w).io.deq.uop.uopc === uopXPACM)
    {
      printf("YH+ Found uopXPACM at decode stage!\n")
    }
      .elsewhen (decode_units(w).io.deq.uop.uopc === uopBNDSTR)
    {
      printf("YH+ Found uopBNDSTR at decode stage!\n")
    }
      .elsewhen (decode_units(w).io.deq.uop.uopc === uopBNDCLR)
    {
      printf("YH+ Found uopBNDCLR at decode stage!\n")
    }
      .elsewhen (decode_units(w).io.deq.uop.uopc === uopBNDSRCH)
    {
      printf("YH+ Found uopBNDSRCH at decode stage!\n")
    }

    //when (decode_units(w).io.deq.uop.csr_cmd === CSR.W)
    //{
    //  printf("YH+ Found CSR.W at decode stage!\n")
    //}
    //	.elsewhen (decode_units(w).io.deq.uop.csr_cmd === CSR.S)
    //{
    //  printf("YH+ Found CSR.S at decode stage!\n")
    //}
    //	.elsewhen (decode_units(w).io.deq.uop.csr_cmd === CSR.C)
    //{
    //  printf("YH+ Found CSR.C at decode stage!\n")
    //}
    //yh+end
  }
  ...
  val dis_hazards = (0 until coreWidth).map(w =>
                      ...
                      //yh+begin
                      || io.lsu.mcq_full(w) && (dis_uops(w).uses_ldq || dis_uops(w).uses_stq)
                      || io.lsu.bdq_full(w) && dis_uops(w).uses_bdq
                      //yh+end
  ...
  for (w <- 0 until coreWidth) {
    ...
    //yh+begin
    dis_uops(w).mcq_idx := io.lsu.dis_mcq_idx(w)
    dis_uops(w).bdq_idx := io.lsu.dis_bdq_idx(w)
    //yh+end
  }
...
//   "CSRFile is being written to spuriously.")
  ...
  io.lsu.wyfy_config.num_bndstr_fail    := custom_csrs.num_bndstr_fail
  io.lsu.wyfy_config.num_bndclr_fail    := custom_csrs.num_bndclr_fail
  io.lsu.wyfy_config.num_cache_hit      := custom_csrs.num_cache_hit
  io.lsu.wyfy_config.num_cache_miss     := custom_csrs.num_cache_miss

  rob.io.wyfy_config.enableWYFY         := custom_csrs.enableWYFY
  rob.io.wyfy_config.num_pacma          := custom_csrs.num_pacma
  rob.io.wyfy_config.num_xpacm          := custom_csrs.num_xpacm

  csr.io.wyfyConfig         := Cat(io.lsu.bndstr_failed, custom_csrs.enableWYFY)
  csr.io.hbt_base_addr      := custom_csrs.hbt_base_addr
  csr.io.hbt_num_way        := custom_csrs.hbt_num_way
  csr.io.num_signed_inst    := io.lsu.num_signed_inst
  csr.io.num_unsigned_inst  := io.lsu.num_unsigned_inst
  csr.io.num_bndstr         := io.lsu.num_bndstr
  csr.io.num_bndclr         := io.lsu.num_bndclr
  csr.io.num_bndsrch        := io.lsu.num_bndsrch
  csr.io.num_bndstr_fail    := io.lsu.num_bndstr_fail
  csr.io.num_bndclr_fail    := io.lsu.num_bndclr_fail
  csr.io.num_cache_hit      := io.lsu.num_cache_hit
  csr.io.num_cache_miss     := io.lsu.num_cache_miss
  csr.io.num_pacma          := rob.io.num_pacma
  csr.io.num_xpacm          := rob.io.num_xpacm
  //yh+end
