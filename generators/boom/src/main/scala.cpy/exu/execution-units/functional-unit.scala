  ...
  with HasBoomUOP
{
  ...
  //yh-val addr = UInt((vaddrBits+1).W) // only for maddr -> LSU
  val addr = UInt(xLen.W) // only for maddr -> LSU //yh+
  ...
  with boom.ifu.HasBoomFrontendParameters
{
  ...
  alu.io.dw  := uop.ctrl.fcn_dw
	...
	//yh+begin
	val is_pa = (uop.ctrl.op_fcn === FN_PACMA) || (uop.ctrl.op_fcn === FN_XPACM)
	//yh+end
...
//   io.resp.bits.data := reg_data
  ...
  val alu_out = Mux(io.req.bits.uop.is_sfb_shadow && io.req.bits.pred_data,
    ...
    Mux(io.req.bits.uop.uopc === uopMOV && !is_pa, io.req.bits.rs2_data, alu.io.out)) //yh+
    //yh-Mux(io.req.bits.uop.uopc === uopMOV, io.req.bits.rs2_data, alu.io.out))

  ////yh+begin
  //printf("YH+ is_sfb_shadow: %d pred_data: %d ldst_is_rs1: %d alu.io.out: %x alu_out: %x rs2_data: %x\n",
  //        io.req.bits.uop.is_sfb_shadow, io.req.bits.pred_data, io.req.bits.uop.ldst_is_rs1, alu.io.out, alu_out, io.req.bits.rs2_data)
  //when (io.req.bits.uop.uopc === uopMOV)
  //{
  //  printf("YH+ Found uopc === uopMOV\n")
  //}
  ////yh+end
  ...
  with freechips.rocketchip.rocket.constants.ScalarOpConstants
{
  ...
  //yh-val sum = (io.req.bits.rs1_data.asSInt + io.req.bits.uop.imm_packed(19,8).asSInt).asUInt
  ...
  //yh+begin
  val sum = Mux(io.req.bits.uop.uopc === uopBNDSRCH, io.req.bits.rs1_data,
              (io.req.bits.rs1_data.asSInt + io.req.bits.uop.imm_packed(19,8).asSInt).asUInt)
  //val sum = (io.req.bits.rs1_data.asSInt + io.req.bits.uop.imm_packed(19,8).asSInt).asUInt
  val ea_sign = Mux(sum(vaddrBits-1), ~sum(45,vaddrBits) === 0.U,
                                       sum(45,vaddrBits) =/= 0.U)
  val effective_address = Cat(io.req.bits.rs1_data(63, vaddrBits+1),
  ...
  when (io.req.valid)
  {
    when (io.req.bits.uop.uses_bdq)
    {
      printf("[1] YH+ rs1_data: %x rs2_data: %x, effective_address: %x store_data: %x\n",
              io.req.bits.rs1_data, io.req.bits.rs2_data, effective_address, store_data)
    }
      .elsewhen (io.req.bits.uop.uses_ldq)
    {
      printf("[2] YH+ rs1_data: %x rs2_data: %x, effective_address: %x store_data: %x\n",
              io.req.bits.rs1_data, io.req.bits.rs2_data, effective_address, store_data)
    }
      .otherwise
    {
      printf("[3] YH+ rs1_data: %x rs2_data: %x, effective_address: %x store_data: %x\n",
              io.req.bits.rs1_data, io.req.bits.rs2_data, effective_address, store_data)
    }
  }
	//yh+end
