  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val dequeue_bdq = WireInit(false.B)
  //val live_bnd_mask = RegInit(0.U(numBdqEntries.W))
  //var next_live_bnd_mask = Mux(dequeue_bdq, live_bnd_mask & ~(1.U << bdq_head),
  //                                            live_bnd_mask)
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    when (dis_bq_val)
    {
      ...
      //bdq(bd_enq_idx).bits.execute_ignore := false.B
      ...
      .elsewhen (dis_mq_val)
    {
      ...
      //mcq(mc_enq_idx).bits.bnd_dep_mask   := next_live_bnd_mask

      mcq(mc_enq_idx).bits.executed       := false.B
      //mcq(mc_enq_idx).bits.execute_ignore := false.B
      ...
      //printf("YH+ [%d] Dispatch mcq(%d) dep_mask: %d\n",
      //        io.core.tsc_reg, mc_enq_idx, next_live_bnd_mask)
    }

    //next_live_bnd_mask = Mux(dis_bq_val & !dis_bndsrch,
    //                          next_live_bnd_mask | (1.U << bd_enq_idx),
    //                          next_live_bnd_mask)
  ...
  val bdq_store_val = (bdq_store_idx === bdq_head
											&& bdq_store_e.valid
                      && (bdq_store_e.bits.state === b_bndStr)
                      && bdq_store_e.bits.committed
                      && !bdq_store_e.bits.executed)

  val bdq_resp_idx = RegNext(AgePriorityEncoder((0 until numBdqEntries).map(i => {
    val e = bdq(i).bits
    e.state === b_bndResp || e.state === b_fail
  }), bdq_head))
  val bdq_resp_e = bdq(bdq_resp_idx)

  val bdq_resp_val = widthMap(w => bdq_resp_e.valid         &&
                      (bdq_resp_e.bits.state === b_bndResp ||
											bdq_resp_e.bits.state === b_fail)     &&
                                    (w == memWidth-1).B)
	...
	when (bdq_resp_val(memWidth-1))
	{
	  printf("YH+ [%d] bdq_resp_idx: %d bdq_resp_val: %d\n",
						io.core.tsc_reg, bdq_resp_idx, bdq_resp_val(memWidth-1))
	}
  ...
  for (w <- 0 until memWidth)
  {
    when (exe_mcq_val(w))
    {
      ...
      mcq(midx).bits.state        := Mux(exe_mcq_isPACed(w), m_bndChk, m_done) // Go to m_bndChk
			mcq(midx).bits.signed       := (exe_mcq_vaddr(w)(45,vaddrBits) === 0.U) && exe_mcq_isPACed(w)
  ...
  // Mask gen.
  val pac_match_mcq = Wire(Vec(numMcqEntries, Bool()))
  ...
  val mcq_load_val = (mcq_load_e.valid
                      ...
                      //&& (mcq_load_e.bits.bnd_dep_mask.asUInt === 0.U))
  ...
  val bnd_load_pac = Mux(mcq_load_val, mcq_load_e.bits.addr(xLen-1,xLen-pacWidth),
                        Mux(bdq_load_val, bdq_load_e.bits.addr.bits(xLen-1,xLen-pacWidth), 0.U))
  ...
  val bnd_store_paddr = (hbt_base_addr + (bnd_store_pac << 5) + (bdq_store_e.bits.count << 3))
  ...
  val bnd_store_uop = Mux(bdq_store_val, bdq_store_e.bits.uop, NullMicroOp)
	val bnd_store_data = Mux(bdq_store_e.bits.uop.uopc === uopBNDCLR,
													0.U, bdq_store_e.bits.data)

	val bnd_resp_uop = bdq_resp_e.bits.uop
	val bnd_resp_val = Mux(bdq_resp_e.bits.state === b_bndResp, 1.U, 0.U)
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  for (w <- 0 until memWidth) {
      ...
      .elsewhen (will_fire_bnd_load(w))
    {
      ...
      //bdq_load_e.bits.executed        := Mux(!mcq_load_val & bdq_load_val, dmem_req_fire(w), 0.U)
      mcq_load_e.bits.executed        := Mux(mcq_load_val, dmem_req_fire(w),
																							mcq_load_e.bits.executed)
      bdq_load_e.bits.executed        := Mux(!mcq_load_val & bdq_load_val, dmem_req_fire(w),
																							bdq_load_e.bits.executed)
      ...
      when (dmem_req_fire(w) && will_fire_bnd_load(w) && mcq_load_val)
      {
        ...
        .elsewhen (dmem_req_fire(w) && will_fire_bnd_load(w) && !mcq_load_val && bdq_load_val)
      {
  ...
  for (w <- 0 until memWidth) {
    ...
    when (io.dmem.resp(w).valid)
    {
        ...
        .elsewhen (io.dmem.resp(w).bits.uop.uses_bdq)
      {
        ...
        when (bdq(bdq_resp_idx).bits.state === b_bndStr)
        {
      ...
      .elsewhen (!dmem_resp_fired(w) && wb_forward_valid(w))
    {
      ...
      when (data_ready && live) {
		...
		//yh+begin
			.elsewhen (!dmem_resp_fired(w) && bdq_resp_val(w))
		{
			printf("YH+ [%d] Found bdq_resp_val! bdq_resp_idx: %d bnd_resp_val: %d\n",
							io.core.tsc_reg, bdq_resp_idx, bnd_resp_val)

      val bnd_uop     = bdq(bdq_resp_idx).bits.uop
      val live        = !IsKilledByBranch(io.core.brupdate, bnd_uop)

      io.core.exe(w).iresp.valid := live
      io.core.exe(w).iresp.bits.uop  := bnd_resp_uop
      io.core.exe(w).iresp.bits.data := bnd_resp_val
  ...
  //yh+begin
  //val fired_bnd_store = RegNext(will_fire_bnd_store(memWidth-1))

  //// We fired bounds store last cycle
  //val do_bnd_search = fired_bnd_store

  //val fired_bnd_pac = RegNext(bnd_store_pac)
  //val fired_bnd_idx  = RegNext(bdq_store_idx)

  //for (i <- 0 until numBdqEntries) {
  //  val b_valid = bdq(i).valid
  //  val b_bits  = bdq(i).bits
  //  val b_addr  = bdq(i).bits.addr

  //  val pac_match = (fired_bnd_pac === b_addr(xLen-1,xLen-pacWidth))

	//	when (do_bnd_search                      		&&
	//				b_valid                               &&
	//				IsOlder(fired_bnd_idx, i.U, bdq_head) &&
	//				pac_match) {

	//		printf("YH+ [%d] Found PAC match bdq(%d) <-> bdq(%d)\n",
	//						io.core.tsc_reg, bdq_head, i.U)

	//		when (b_bits.state === b_occChk && b_bits.executed) {
	//			printf("YH+ [%d] Do replay (1) bdq(%d)\n", io.core.tsc_reg, i.U)
	//			bdq(i).bits.execute_ignore  := true.B   // ignore next response
  //      bdq(i).bits.count           := 0.U
	//		} .elsewhen (b_bits.state =/= b_init) {
	//			printf("YH+ [%d] Do replay (2) bdq(%d)\n", io.core.tsc_reg, i.U)
	//			bdq(i).bits.state           := b_occChk // Do replay
	//			bdq(i).bits.execute_ignore  := true.B   // TODO this is not the best but safe...
	//			bdq(i).bits.uop.mem_cmd 		:= rocket.M_XRD
  //      bdq(i).bits.count           := 0.U
	//		}
	//	}
  //}
  ...
  val bdq_load_resp_val = RegNext(widthMap(w =>
                                  Mux(io.dmem.resp(w).valid,
		                                  (io.dmem.resp(w).bits.uop.uses_bdq &&
																			io.dmem.resp(w).bits.uop.mem_cmd === rocket.M_XRD), false.B)))
  ...
  val bdq_load_resp_data = RegNext(widthMap(w =>
                                  Mux(io.dmem.resp(w).valid && io.dmem.resp(w).bits.uop.uses_bdq,
                                      io.dmem.resp(w).bits.data, 0.U)))
  val bdq_load_resp_bndstr = RegNext(widthMap(w =>
                                  Mux(io.dmem.resp(w).valid,
                                      (io.dmem.resp(w).bits.uop.uopc === uopBNDSTR), false.B)))

  val bdq_load_resp_bndclr = RegNext(widthMap(w =>
                                  Mux(io.dmem.resp(w).valid,
                                      (io.dmem.resp(w).bits.uop.uopc === uopBNDCLR), false.B)))

  val bdq_load_resp_bndsrch = RegNext(widthMap(w =>
                                  Mux(io.dmem.resp(w).valid,
  ...
  for (w <- 0 until memWidth) {
    ...
    when (mcq_load_resp_val(w) && mcq_live)
    {
    ...
    when (bdq_load_resp_val(w) && bdq_live)
    {
      ...
      //val execute_ignore = bdq(bdq_idx).bits.execute_ignore
      //val occ_chk = Mux(bdq_load_resp_bndstr(w), true.B, //TODO
      val occ_chk = Mux(bdq_load_resp_bndstr(w), bdq_load_resp_data(w) === 0.U, //TODO
      ...
      //  bdq(bdq_idx).bits.execute_ignore  := false.B
      //  bdq(bdq_idx).bits.count           := 0.U
      //}
      when (occ_chk)
      {
  ...
  //val bnd_brkilled_mask = Wire(Vec(numBdqEntries, Bool()))
  for (i <- 0 until numBdqEntries)
  {
    //bnd_brkilled_mask(i) := false.B
  ...
  when (dequeue_mcq)
  {
    ...
    mcq_head := Mux(dequeue_mcq, WrapInc(mcq_head, numMcqEntries),
                                    mcq_head)
  }

  val bdq_head_e  = bdq(bdq_head) // Current MCQ entry to operate with

  dequeue_bdq := (bdq_head_e.valid
                      && bdq_head_e.bits.committed
                      && (bdq_head_e.bits.state === b_done ||
                          bdq_head_e.bits.state === b_fail))

  when (dequeue_bdq)
  {
    bdq_head_e.valid              := false.B
    bdq_head_e.bits.addr.valid    := false.B
    bdq_head_e.bits.state         := b_init

    printf("YH+ [%d] Dequeue bdq(%d)\n", io.core.tsc_reg, bdq_head)

    bdq_head := Mux(dequeue_bdq, WrapInc(bdq_head, numBdqEntries),
                                    bdq_head)
  }
  ...
  //  when (dequeue_bdq)
  //  {
  //    mcq(i).bits.bnd_dep_mask := mcq(i).bits.bnd_dep_mask & ~(1.U << bdq_head)
	//		printf("YH+ [%d] Update bnd_dep_mask: %d mcq(%d)\n",
	//						io.core.tsc_reg, mcq(i).bits.bnd_dep_mask & ~(1.U << bdq_head), i.U)
  //  }
  //}
  ...
  {
    when (bdq_head_e.bits.uop.uopc === uopBNDSTR)
    {
      num_bndstr          := num_bndstr + 1.U

      when (bdq_head_e.bits.state === b_fail)
      {
        num_bndstr_fail   := num_bndstr_fail + 1.U
      }
    }
      .elsewhen (bdq_head_e.bits.uop.uopc === uopBNDCLR)
    {
      ...
      .elsewhen (bdq_head_e.bits.uop.uopc === uopBNDSRCH)
    {
  ...
  //yh+begin
  // for the live_bnd_mask, need to kill bounds stores that haven't been committed
  //val bnd_exc_killed_mask = WireInit(VecInit((0 until numBdqEntries).map(x=>false.B)))
  when (reset.asBool || io.core.exception)
  {
    mcq_head := 0.U
    mcq_tail := 0.U
  ...
  ////yh+begin
  //live_bnd_mask := next_live_bnd_mask &
  //                  ~(bnd_brkilled_mask.asUInt) &
  //                  ~(bnd_exc_killed_mask.asUInt)
  ////yh+end
}
