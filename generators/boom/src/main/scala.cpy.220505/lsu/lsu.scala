...
//yh+begin
class BDQExeIO(implicit p: Parameters) extends BoomBundle()(p)
{
  // The "resp" of the baddrcalc is really a "req" to the LSU
  val req       = Flipped(new ValidIO(new FuncUnitResp(xLen)))
}
//yh+end
...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)
{
  ...
  val perf = Input(new Bundle {
	...
	//yh+begin
	val enableWYFY					= Output(Bool())
  val num_cache_hit_in    = Output(UInt(xLen.W))
  val num_cache_miss_in  	= Output(UInt(xLen.W))
  val num_cache_hit_out  	= Input(UInt(xLen.W))
  val num_cache_miss_out	= Input(UInt(xLen.W))
	//yh+end
}
...
class LSUCoreIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val bdq_exe = Vec(intWidth, new BDQExeIO) //yh+
  ...
  val wyfy_config = Input(new LSUWYFYConfig)
	...
	val bndstr_failed = Output(Bool())
  val num_signed_inst    = Output(UInt(xLen.W))
  val num_unsigned_inst  = Output(UInt(xLen.W))
  val num_bndstr        = Output(UInt(xLen.W))
  val num_bndclr        = Output(UInt(xLen.W))
  val num_bndsrch       = Output(UInt(xLen.W))
  val num_bndstr_fail   = Output(UInt(xLen.W))
  val num_bndclr_fail   = Output(UInt(xLen.W))
  val num_cache_hit      = Output(UInt(xLen.W))
  val num_cache_miss     = Output(UInt(xLen.W))
  //yh+end
...
class BDQEntry(implicit p: Parameters) extends BoomBundle()(p)
    with HasBoomUOP
{
  val addr                = Valid(UInt(xLen.W)) // Pointer adress of instruction
  val data                = UInt(xLen.W)

  val executed            = Bool() // Bounds load executed committed to memory
  val committed           = Bool() // committed by ROB

  val way                 = UInt(hbtAssocWidth.W) // The way to access in a row of the HBT 
  var count               = UInt(hbtAssocWidth.W) // Count of failed attempts to access in a bounds-checking operation
    ...
    with HasBoomUOP
{
  ...
  val youngest_bdq_idx    = UInt(bdqAddrSz.W) // index of the oldest store younger than us
  //val bnd_dep_mask        = UInt(numBdqEntries.W) // list of bounds stores older than us
...
class LSUWYFYConfig(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val num_bndstr_fail     = UInt(xLen.W)
  val num_bndclr_fail     = UInt(xLen.W)
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val mcq_commit_head  = Reg(UInt(mcqAddrSz.W)) // point to next bounds store to commit
  val bdq_head         = Reg(UInt(bdqAddrSz.W)) 
  val bdq_tail         = Reg(UInt(bdqAddrSz.W))
  val bdq_commit_head  = Reg(UInt(bdqAddrSz.W)) // point to next bounds store to commit

  val m_init :: m_bndChk :: m_fail :: m_done :: Nil = Enum(4)
  val b_init :: b_occChk :: b_bndStr :: b_bndResp :: b_fail :: b_done :: Nil = Enum(6)
  //yh+end
  ...
  //val will_fire_bnd_incoming   = Wire(Vec(memWidth, Bool())) //yh+
  ...
  val exe_req = WireInit(VecInit(io.core.exe.map(_.req)))
	//yh+begin
  val bdq_exe_req = WireInit(VecInit(io.core.bdq_exe.map(_.req)))
	//for (w <- 0 until memWidth) {
	//	assert(!(exe_req(w).valid && bdq_exe_req(w).valid))
	//}
	//yh+end
  ...
  //-------------------------------------------------------------
  //-------------------------------------------------------------
  val enableWYFY          = RegInit(false.B)
  val initWYFY            = RegInit(false.B)
  val hbt_base_addr       = Reg(UInt(coreMaxAddrBits.W))
  val hbt_num_way         = Reg(UInt(hbtAssocWidth.W))

	val bndstr_failed				= RegInit(false.B)
  val num_signed_inst     = RegInit(0.U(xLen.W))
  val num_unsigned_inst   = RegInit(0.U(xLen.W))
  val num_bndstr          = RegInit(0.U(xLen.W))
  val num_bndclr          = RegInit(0.U(xLen.W))
  val num_bndsrch         = RegInit(0.U(xLen.W))
  val num_bndstr_fail     = RegInit(0.U(xLen.W))
  val num_bndclr_fail     = RegInit(0.U(xLen.W))

  val num_cache_hit       = RegInit(0.U(xLen.W))
  val num_cache_miss      = RegInit(0.U(xLen.W))
  ...
  hbt_num_way             := io.core.wyfy_config.hbt_num_way(hbtAssocWidth-1,0)

	io.core.bndstr_failed			:= bndstr_failed
  io.core.num_signed_inst  	:= num_signed_inst
  io.core.num_unsigned_inst := num_unsigned_inst
  io.core.num_bndstr       	:= num_bndstr
  io.core.num_bndclr       	:= num_bndclr
  io.core.num_bndsrch      	:= num_bndsrch
  io.core.num_bndstr_fail   := num_bndstr_fail
  io.core.num_bndclr_fail   := num_bndclr_fail
 
	io.dmem.enableWYFY				:= io.core.wyfy_config.enableWYFY
	io.dmem.num_cache_hit_in 	:= io.core.wyfy_config.num_cache_hit
  io.dmem.num_cache_miss_in	:= io.core.wyfy_config.num_cache_miss

  io.core.num_cache_hit     := io.dmem.num_cache_hit_out
  io.core.num_cache_miss    := io.dmem.num_cache_miss_out
  ...
  //val live_bnd_mask = RegInit(0.U(numBdqEntries.W))
  val bdq_nonempty = (0 until numBdqEntries).map{ i => bdq(i).valid }.reduce(_||_) =/= 0.U
  var bd_enq_idx = bdq_tail
  var bdq_full = Bool()

  val mcq_nonempty = (0 until numMcqEntries).map{ i => mcq(i).valid }.reduce(_||_) =/= 0.U
  var mc_enq_idx = mcq_tail
  var mcq_full = Bool()

  for (w <- 0 until coreWidth)
  {
    bdq_full = WrapInc(bd_enq_idx, numBdqEntries) === bdq_head
    io.core.bdq_full(w)    := bdq_full
    io.core.dis_bdq_idx(w) := bd_enq_idx

    mcq_full = WrapInc(mc_enq_idx, numMcqEntries) === mcq_head
    io.core.mcq_full(w)    := mcq_full
    io.core.dis_mcq_idx(w) := mc_enq_idx
    ...
    val dis_mq_val = (io.core.dis_uops(w).valid &&
												io.core.dis_uops(w).bits.uses_mcq &&
                        !io.core.dis_uops(w).bits.exception)

    when (dis_bq_val)
    {
      bdq(bd_enq_idx).valid               := true.B
      bdq(bd_enq_idx).bits.addr.valid     := false.B
      bdq(bd_enq_idx).bits.uop            := io.core.dis_uops(w).bits
      bdq(bd_enq_idx).bits.uop.mem_cmd    := rocket.M_XRD
      bdq(bd_enq_idx).bits.uop.mem_size   := 3.U
      //bdq(bd_enq_idx).bits.uop.mem_signed := false.B
      bdq(bd_enq_idx).bits.uop.is_bndload := true.B
      bdq(bd_enq_idx).bits.executed       := false.B
      bdq(bd_enq_idx).bits.committed      := false.B

      bdq(bd_enq_idx).bits.way            := 0.U
      bdq(bd_enq_idx).bits.count          := 0.U
      bdq(bd_enq_idx).bits.state          := b_init

      printf("YH+ [%d] Dispatch bdq(%d)\n", io.core.tsc_reg, bd_enq_idx)
    }
      .elsewhen (dis_mq_val)
    {
      mcq(mc_enq_idx).valid               := true.B
      mcq(mc_enq_idx).bits.uop            := io.core.dis_uops(w).bits
      mcq(mc_enq_idx).bits.uop.mem_cmd    := rocket.M_XRD
      mcq(mc_enq_idx).bits.uop.mem_size   := 3.U // TODO can handle at decode
      mcq(mc_enq_idx).bits.uop.mem_signed := false.B // TODO can handle at decode
      mcq(mc_enq_idx).bits.uop.uses_ldq   := false.B
      mcq(mc_enq_idx).bits.uop.uses_stq   := false.B
      mcq(mc_enq_idx).bits.uop.is_bndload := true.B
      mcq(mc_enq_idx).bits.youngest_bdq_idx := bd_enq_idx

      mcq(mc_enq_idx).bits.executed       := false.B
      mcq(mc_enq_idx).bits.committed      := false.B
      mcq(mc_enq_idx).bits.signed         := false.B

      mcq(mc_enq_idx).bits.way            := 0.U
      mcq(mc_enq_idx).bits.count          := 0.U
      mcq(mc_enq_idx).bits.state          := m_init

      printf("YH+ [%d] Dispatch mcq(%d)\n", io.core.tsc_reg, mc_enq_idx)
    }

    bd_enq_idx = Mux(dis_bq_val, WrapInc(bd_enq_idx, numBdqEntries),
                                 bd_enq_idx)

    mc_enq_idx = Mux(dis_mq_val, WrapInc(mc_enq_idx, numMcqEntries),
                                 mc_enq_idx)
  }
  ...
  var temp_bdq_commit_head = bdq_commit_head
  var temp_mcq_commit_head = mcq_commit_head
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    val commit_mcq = (io.core.commit.valids(w) &&
												io.core.commit.uops(w).uses_mcq)

    val bidx = io.core.commit.uops(w).bdq_idx
    val midx = io.core.commit.uops(w).mcq_idx

    when (commit_bdq)
    {
      printf("YH+ [%d] Committed bdq(%d) bdq_commit_head: %d\n",
              io.core.tsc_reg, io.core.commit.uops(w).bdq_idx, bdq_commit_head)
      bdq(bidx).bits.committed := true.B
    }

    when (commit_mcq)
    {
      printf("YH+ [%d] Committed mcq(%d) mcq_commit_head: %d\n",
              io.core.tsc_reg, io.core.commit.uops(w).mcq_idx, mcq_commit_head)
      mcq(midx).bits.committed := true.B
    }

    temp_bdq_commit_head = Mux(commit_bdq,
                               WrapInc(temp_bdq_commit_head, numBdqEntries),
                               temp_bdq_commit_head)

    temp_mcq_commit_head = Mux(commit_mcq,
                               WrapInc(temp_mcq_commit_head, numMcqEntries),
                               temp_mcq_commit_head)
  }
  ...
  for (i <- 0 until numBdqEntries)
  {
    ...
    {
        when (!bdq(i).bits.committed)
        {
          bdq(i).valid            := false.B
          bdq(i).bits.addr.valid  := false.B
          bdq(i).bits.state       := b_init
          //bnd_exc_killed_mask(i)  := true.B
        }
          .elsewhen (bdq(i).bits.state === b_occChk)
        {
          bdq(i).bits.executed    := false.B
        }
    }
      .elsewhen (bdq(i).valid)
    {
      bdq(i).bits.uop.br_mask := GetNewBrMask(io.core.brupdate, bdq(i).bits.uop.br_mask)

      when (IsKilledByBranch(io.core.brupdate, bdq(i).bits.uop))
      {
        printf("YH+ [%d] Killed by misprediction bdq(%d)\n",
                io.core.tsc_reg, i.U)
        ...
        assert(!bdq(i).bits.committed)
      }
        .elsewhen (i.U === bdq_head &&
                  bdq(i).bits.committed &&
                  (bdq(i).bits.state === b_done ||
                  bdq(i).bits.state === b_fail))
      {
        bdq(i).valid              := false.B
        bdq(i).bits.addr.valid    := false.B
        bdq(i).bits.state         := b_init
  ...
  for (i <- 0 until numMcqEntries)
  {
      ...
      .elsewhen (io.core.exception)
    {
      when (!mcq(i).bits.committed)
      {
        ...
        .elsewhen (mcq(i).bits.state === m_bndChk)
      {
        mcq(i).bits.executed    := false.B
      }
    }
      .elsewhen (mcq(i).valid)
    {
      mcq(i).bits.uop.br_mask := GetNewBrMask(io.core.brupdate, mcq(i).bits.uop.br_mask)

      when (IsKilledByBranch(io.core.brupdate, mcq(i).bits.uop))
      {
        printf("YH+ [%d] Killed by misprediction mcq(%d)\n",
                io.core.tsc_reg, i.U)

        mcq(i).valid            := false.B
        mcq(i).bits.state       := m_init

        assert(!mcq(i).bits.committed)
      }
        .elsewhen (i.U === mcq_head &&
                  mcq(i).bits.committed &&
                  (mcq(i).bits.state === m_done ||
                  mcq(i).bits.state === m_fail))
      {
        mcq(i).valid       := false.B
        mcq(i).bits.state  := m_init
  ...
  // Clear
  val bdq_clear = (bdq(bdq_head).valid &&
              (bdq(bdq_head).bits.state === b_done ||
              bdq(bdq_head).bits.state === b_fail) &&
              !io.core.exception &&
              bdq(bdq_head).bits.committed)

  val mcq_clear = (mcq(mcq_head).valid &&
              (mcq(mcq_head).bits.state === m_done ||
              mcq(mcq_head).bits.state === m_fail) &&
              !io.core.exception &&
              mcq(mcq_head).bits.committed)

  when (reset.asBool) {
    bdq_head := 0.U
  } .elsewhen (bdq_clear) {
    bdq_head := WrapInc(bdq_head, numBdqEntries)
  }

  when (reset.asBool) {
    mcq_head := 0.U
  } .elsewhen (mcq_clear) {
    mcq_head := WrapInc(mcq_head, numMcqEntries)
  }
  ...
  when (reset.asBool)
  {
    ...
    .elsewhen (io.core.exception)
  {
    printf("YH+ [%d] Exception occurred! mcq_commit_head: %d bdq_commit_head: %d\n",
            io.core.tsc_reg, mcq_commit_head, bdq_commit_head)

    bdq_tail := bdq_commit_head
    bdq_commit_head := temp_bdq_commit_head
    mcq_tail := mcq_commit_head
    mcq_commit_head := temp_mcq_commit_head
  }
    .elsewhen (io.core.brupdate.b2.mispredict)
  {
    printf("YH+ [%d] Misprediction mcq_idx: %d bdq_idx: %d\n",
            io.core.tsc_reg, io.core.brupdate.b2.uop.mcq_idx, io.core.brupdate.b2.uop.bdq_idx)

    bdq_tail := io.core.brupdate.b2.uop.bdq_idx
    bdq_commit_head := temp_bdq_commit_head
    mcq_tail := io.core.brupdate.b2.uop.mcq_idx
    mcq_commit_head := temp_mcq_commit_head
  } .otherwise {
    bdq_tail := bd_enq_idx
    bdq_commit_head := temp_bdq_commit_head
  ...
  for (w <- 0 until intWidth)
	{
		when (bdq_exe_req(w).valid)
		{
      assert(bdq_exe_req(w).bits.uop.uses_bdq)

			val bidx = bdq_exe_req(w).bits.uop.bdq_idx

			bdq(bidx).bits.addr.valid   := true.B
			bdq(bidx).bits.addr.bits    := bdq_exe_req(w).bits.addr
			bdq(bidx).bits.data         := Cat(bdq_exe_req(w).bits.data(31,0), bdq_exe_req(w).bits.addr(31,0))
			bdq(bidx).bits.state        := b_done // Go to b_occChk
			//bdq(bidx).bits.state        := b_occChk // Go to b_occChk

			printf("YH+ [%d] Exe bdq(%d) bdq_exe_req vaddr: %x PAC: %d size: %d\n",
			  io.core.tsc_reg, bidx, bdq_exe_req(w).bits.addr, bdq_exe_req(w).bits.addr(xLen-1,xLen-pacWidth), bdq_exe_req(w).bits.data)
		}
	}

  val bdq_bnd_idx = RegNext(AgePriorityEncoder((0 until numBdqEntries).map(i => {
    val e = bdq(i).bits
    e.addr.valid && !e.executed
  }), bdq_head))
  val bdq_bnd_e = bdq(bdq_bnd_idx)

  //TODO // Mask gen.
  //TODO val pac_match_bdq = Wire(Vec(numBdqEntries, Bool()))
  //TODO val bdq_load_pac = bdq_bnd_e.bits.addr.bits(xLen-1,xLen-pacWidth)

  //TODO for (i <- 0 until numBdqEntries) {
  //TODO   val temp_pac = bdq(i).bits.addr.bits(xLen-1,xLen-pacWidth)

  //TODO   pac_match_bdq(i) := bdq_bnd_e.valid &&
  //TODO                 IsOlder(i.U, bdq_bnd_idx, bdq_head) &&
  //TODO                 (bdq(i).bits.state =/= b_done) &&
  //TODO                 (!bdq(i).bits.addr.valid ||
  //TODO                   temp_pac === bdq_load_pac)

  //TODO   when (bdq_bnd_e.valid && IsOlder(i.U, bdq_bnd_idx, bdq_head))
  //TODO   {
  //TODO     printf("YH+ [%d] pac_match_bdq bdq(%d): %d <-> bdq(%d): %d\n",
  //TODO             io.core.tsc_reg, i.U, temp_pac, bdq_bnd_idx, bdq_load_pac)
  //TODO   }
  //TODO }

  val bdq_load_val = (bdq_bnd_e.valid                    	&&
											bdq_bnd_e.bits.addr.valid						&&
                      !bdq_bnd_e.bits.executed						&&
                      bdq_bnd_idx === bdq_head            && //TODO
                      //TODO !pac_match_bdq.reduce(_|_)          &&
                      bdq_bnd_e.bits.state === b_occChk)

  //TODO when (bdq_bnd_e.valid											&&
	//TODO 				bdq_bnd_e.bits.addr.valid					&&
  //TODO         bdq_bnd_e.bits.state === b_occChk &&
  //TODO         !bdq_bnd_e.bits.executed)
  //TODO {
  //TODO   when (pac_match_bdq.reduce(_|_))
  //TODO   {
  //TODO     printf("YH+ [%d] pac_match_bdq bdq(%d) prevented bounds load!\n",
  //TODO             io.core.tsc_reg, bdq_bnd_idx)
  //TODO   }
  //TODO }

  val bdq_store_val = (bdq_bnd_e.valid                 	&&
											bdq_bnd_e.bits.addr.valid					&&
                      !bdq_bnd_e.bits.executed					&&
                      bdq_bnd_idx === bdq_head          &&
                      bdq_bnd_e.bits.state === b_bndStr &&
                      bdq_bnd_e.bits.committed)

	when (bdq_load_val)
	{
	  printf("YH+ [%d] bdq_bnd_idx: %d bdq_load_val: %d\n",
						io.core.tsc_reg, bdq_bnd_idx, bdq_load_val)
	}

  val bdq_resp_val = Wire(Vec(memWidth, Bool()))
	bdq_resp_val := widthMap(w => bdq_bnd_e.valid         &&
											bdq_bnd_e.bits.addr.valid					&&
											bdq_bnd_e.bits.uop.uopc === uopBNDSRCH &&
                      bdq_bnd_idx === bdq_head					&&
											(bdq_bnd_e.bits.state === b_bndResp ||
											bdq_bnd_e.bits.state === b_fail) &&
											(w == 0).B)

	//val bdq_resp_val = (bdq_bnd_e.valid                 	&&
	//										bdq_bnd_e.bits.addr.valid					&&
	//										bdq_bnd_e.bits.uop.uopc === uopBNDSRCH &&
  //                    bdq_bnd_idx === bdq_head					&&
	//										(bdq_bnd_e.bits.state === b_bndResp ||
  ...
  val exe_mcq_isPACed = widthMap(w => exe_req(w).bits.addr(xLen-1,xLen-pacWidth) =/= 0.U &&
																	(exe_mcq_vaddr(w)(45,vaddrBits) === 0.U))
  ...
  for (w <- 0 until memWidth)
  {
    when (exe_mcq_val(w))
    {
      ...
      mcq(midx).bits.addr         := exe_mcq_vaddr(w)
      mcq(midx).bits.state        := Mux(exe_mcq_isPACed(w), m_bndChk, m_done) // Go to m_bndChk
			mcq(midx).bits.signed       := exe_mcq_isPACed(w)

			when (enableWYFY)
			{
	      printf("YH+ [%d] Exe mcq(%d) exe_req(%d) vaddr: %x PAC: %d PACed: %d\n",
	        io.core.tsc_reg, midx, w.U, exe_mcq_vaddr(w), exe_mcq_vaddr(w)(xLen-1,xLen-pacWidth), exe_mcq_isPACed(w))
			}
    }
  }
  ...
  val mcq_bnd_e = mcq(mcq_load_idx)

  //TODO // Mask gen.
  //TODO val pac_match_mcq = Wire(Vec(numMcqEntries, Bool()))
  //TODO val mcq_load_pac = mcq_bnd_e.bits.addr(xLen-1,xLen-pacWidth)
  //TODO val youngest_bdq_idx = mcq_bnd_e.bits.youngest_bdq_idx

  //TODO for (i <- 0 until numBdqEntries) {
  //TODO   val temp_pac = bdq(i).bits.addr.bits(xLen-1,xLen-pacWidth)

  //TODO   pac_match_mcq(i) := mcq_bnd_e.valid &&
  //TODO                 bdq(i).valid &&
  //TODO                 IsOlder(i.U, youngest_bdq_idx, bdq_head) &&
  //TODO                 bdq(i).bits.state =/= b_done &&
  //TODO                 bdq(i).bits.uop.uopc =/= uopBNDSRCH &&
  //TODO                 (!bdq(i).bits.addr.valid ||
  //TODO                   temp_pac === mcq_load_pac)

  //TODO   when (mcq_bnd_e.valid && IsOlder(i.U, youngest_bdq_idx, bdq_head))
  //TODO   {
  //TODO     printf("YH+ [%d] pac_match_mcq bdq(%d): %d <-> mcq(%d): %d youngest_bdq_idx: %d\n",
  //TODO             io.core.tsc_reg, i.U, temp_pac, mcq_load_idx, mcq_load_pac, youngest_bdq_idx)
  //TODO   }
  //TODO }

  val mcq_load_val = (mcq_bnd_e.valid                        &&
                      mcq_bnd_e.bits.state === m_bndChk      &&
                      //TODO !pac_match_mcq.reduce(_|_)              &&
                      !mcq_bnd_e.bits.executed)

  //TODO when (mcq_bnd_e.valid &&
  //TODO         (mcq_bnd_e.bits.state === m_bndChk) &&
  //TODO         !mcq_bnd_e.bits.executed)
  //TODO {
  //TODO   when (pac_match_mcq.reduce(_|_))
  //TODO   {
  //TODO     printf("YH+ [%d] pac_match_mcq mcq(%d) prevented bounds load!\n",
  //TODO             io.core.tsc_reg, mcq_load_idx)
  //TODO   }
  //TODO }
  ...
  val can_fire_bnd_load = widthMap(w => (bdq_load_val || mcq_load_val)  &&
                                    !lrsc_valid                         &&
                                    !io.core.exception                  &&
                                    (w == memWidth-1).B)
  ...
  //val bnd_load_pac = 0.U
  val bnd_load_pac = Mux(bdq_load_val, bdq_bnd_e.bits.addr.bits(xLen-1,xLen-pacWidth),
											Mux(mcq_load_val, mcq_bnd_e.bits.addr(xLen-1,xLen-pacWidth), 0.U))
  val bnd_load_count = Mux(bdq_load_val, bdq_bnd_e.bits.count, 
												Mux(mcq_load_val, mcq_bnd_e.bits.count, 0.U))
  //val bnd_load_paddr = (hbt_base_addr + Cat((bnd_load_pac << 2), (bnd_load_count << 3))) // assume log2Ceil(hbt_num_way) == 2
  val bnd_load_paddr = (hbt_base_addr + (bnd_load_pac << 5) + (bnd_load_count << 3))
  //val bnd_load_paddr = (hbt_base_addr | (bnd_load_pac << 6) | (bnd_load_count << 3))
  val bnd_load_uop = Mux(bdq_load_val, bdq_bnd_e.bits.uop,
											Mux(mcq_load_val, mcq_bnd_e.bits.uop, NullMicroOp))

  //val bnd_store_pac = 0.U
  val bnd_store_pac = bdq_bnd_e.bits.addr.bits(xLen-1,xLen-pacWidth)
  val bnd_store_count = bdq_bnd_e.bits.count
  //val bnd_store_paddr = (hbt_base_addr + Cat((bnd_store_pac << 2), (bdq_bnd_e.bits.count << 3))) // assume log2Ceil(hbt_num_way) == 2
  val bnd_store_paddr = (hbt_base_addr + (bnd_store_pac << 5) + (bnd_store_count << 3))
  //val bnd_store_paddr = (hbt_base_addr | (bnd_store_pac << 6) | (bdq_bnd_e.bits.count << 3))
  val bnd_store_uop = bdq_bnd_e.bits.uop
	val bnd_store_data = Mux(bdq_bnd_e.bits.uop.uopc === uopBNDCLR, 0.U, bdq_bnd_e.bits.data)

  val bnd_resp_pac = bdq_bnd_e.bits.addr.bits(xLen-1,xLen-pacWidth)
  val bnd_resp_count = bdq_bnd_e.bits.count
  val bnd_resp_paddr = (hbt_base_addr + (bnd_resp_pac << 5) + (bnd_resp_count << 3))
  val bnd_resp_data = Wire(UInt(xLen.W))
  bnd_resp_data := Mux(bdq_bnd_e.bits.state === b_bndResp, 1.U, 0.U)

  when (mcq_load_val)
  {
    printf("YH+ [%d] mcq_load_pac: %d mcq_load_count: %d mcq_load_paddr: %x\n",
            io.core.tsc_reg, bnd_load_pac, bnd_load_count, bnd_load_paddr)
  }
    .elsewhen (bdq_load_val)
  {
    printf("YH+ [%d] bdq_load_pac: %d bdq_load_count: %d bdq_load_paddr: %x\n",
            io.core.tsc_reg, bnd_load_pac, bnd_load_count, bnd_load_paddr)
  }

  when (bdq_store_val)
  {
    printf("YH+ [%d] bdq_store_pac: %d bdq_store_count: %d bdq_store_paddr: %x\n",
            io.core.tsc_reg, bnd_store_pac, bnd_store_count, bnd_store_paddr)
  }

  //io.core.fencei_rdy    := !stq_nonempty && io.dmem.ordered //yh+
  io.core.fencei_rdy    := !stq_nonempty && !mcq_nonempty && !bdq_nonempty && io.dmem.ordered //yh+
  //io.core.fencei_rdy    := !stq_nonempty && ((!mcq_nonempty && !bdq_nonempty) || lrsc_valid) && io.dmem.ordered //yh+
	//printf("YH+ [%d] stq_nonempty: %d mcq_nonempty: %d bdq_nonempty: %d lrsc_valid: %d io.demm.ordered: %d\n",
	//				io.core.tsc_reg, stq_nonempty, mcq_nonempty, bdq_nonempty, lrsc_valid, io.dmem.ordered)
	//printf("YH+ [%d] hbt_base_addr: %x\n", io.core.tsc_reg, hbt_base_addr)
  //yh+end
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  val stq_incoming_e   = widthMap(i => stq(stq_incoming_idx(i)))
	...
	//yh+begin
	//val bdq_incoming_idx = widthMap(i => bdq_exe_req(i).bits.uop.bdq_idx)
	//val bdq_incoming_e   = widthMap(i => bdq(bdq_incoming_idx(i)))
	//yh+end
  ...
  val can_fire_std_incoming  = widthMap(w => exe_req(w).valid && exe_req(w).bits.uop.ctrl.is_std
	...
	//yh+begin
  // Can we fire an incoming bounds addrgen
  //val can_fire_bnd_incoming  = widthMap(w => bdq_exe_req(w).valid && bdq_exe_req(w).bits.uop.uses_bdq
	//																						&& bdq_exe_req(w).bits.uop.uopc =/= uopBNDSRCH) // BNDSRCH will write back later
	//yh+end
  ...
  for (w <- 0 until memWidth) {
    ...
    //will_fire_bnd_incoming  (w) := lsu_sched(can_fire_bnd_incoming  (w) , false, false, false, true)  //     ,    ,      , ROB //yh+
    ...
    will_fire_store_commit  (w) := lsu_sched(can_fire_store_commit  (w) , false, true , false, false) //     , DC
		//yh+begin
    will_fire_bnd_load      (w) := lsu_sched(can_fire_bnd_load      (w) , false, true , false, false) //     , DC ,
    will_fire_bnd_store     (w) := lsu_sched(can_fire_bnd_store     (w) , false, true , false, false) //     , DC ,
		//yh+end
    ...
    assert(!(exe_req(w).valid && !(will_fire_load_incoming(w) ||
                                    ...
                                    will_fire_std_incoming(w) ||
																		//will_fire_bnd_incoming(w) ||
                                    will_fire_sfence(w))))
		//assert(!(bdq_exe_req(w).valid && !will_fire_bnd_incoming(w) && bdq_exe_req(w).bits.uop.uopc =/= uopBNDSRCH))
    //yh+end
  ...
  val exe_tlb_vaddr = widthMap(w =>
                    Mux(will_fire_load_incoming (w) ||
                        ...
                        will_fire_sta_incoming  (w)  , exe_req(w).bits.addr(vaddrBits,0), //yh+ to mask PAC
  ...
  for (w <- 0 until memWidth) {
      ...
      .elsewhen (will_fire_hella_wakeup(w))
    {
      ...
      dmem_req(w).bits.is_hella       := true.B
		...
		//yh+begin
      .elsewhen (will_fire_bnd_load(w))
    {
      dmem_req(w).valid               := true.B
      dmem_req(w).bits.addr           := bnd_load_paddr
      dmem_req(w).bits.uop            := bnd_load_uop

      mcq_bnd_e.bits.executed        	:= Mux(mcq_load_val & !bdq_load_val, dmem_req_fire(w),
																							mcq_bnd_e.bits.executed)
      bdq_bnd_e.bits.executed        	:= Mux(bdq_load_val, dmem_req_fire(w),
																							bdq_bnd_e.bits.executed)

      when (dmem_req_fire(w) && will_fire_bnd_load(w) && mcq_load_val && !bdq_load_val)
      {
        printf("YH+ [%d] Send bounds load mcq(%d) paddr: %x\n",
                io.core.tsc_reg, mcq_load_idx, bnd_load_paddr)
      }
        .elsewhen (dmem_req_fire(w) && will_fire_bnd_load(w) && bdq_load_val)
      {
        printf("YH+ [%d] Send bounds load bdq(%d) paddr: %x\n",
                io.core.tsc_reg, bdq_bnd_idx, bnd_load_paddr)
      }
    }
      .elsewhen (will_fire_bnd_store(w))
    {
      dmem_req(w).valid               := true.B
      dmem_req(w).bits.addr           := bnd_store_paddr
      dmem_req(w).bits.data           := (new freechips.rocketchip.rocket.StoreGen(
                                          bnd_store_uop.mem_size, 0.U,
                                          bnd_store_data,
                                          coreDataBytes)).data
      dmem_req(w).bits.uop            := bnd_store_uop

      bdq_bnd_e.bits.executed       	:= dmem_req_fire(w)

      when (dmem_req_fire(w) && will_fire_bnd_store(w) && bdq_store_val)
      {
        printf("YH+ [%d] Send bounds store bdq(%d) paddr: %x data: %x\n",
                io.core.tsc_reg, bdq_bnd_idx, bnd_store_paddr, bnd_store_data)
      }
    }
		//yh+end
  ...
  val fired_std_incoming   = widthMap(w => RegNext(will_fire_std_incoming (w) && !exe_req_killed(w)))
	//val fired_bnd_incoming	 = widthMap(w => RegNext(will_fire_bnd_incoming (w) && !exe_req_killed(w))) //yh+
  ...
  val mem_paddr                = RegNext(widthMap(w => dmem_req(w).bits.addr))
	...
	//yh+begin
	//val mem_bdq_incoming_e	 = RegNext(widthMap(w => UpdateBrMask(io.core.brupdate, bdq_incoming_e(w))))
	//yh+end
  ...
  for (w <- 0 until memWidth) {
    ...
    when (fired_stad_incoming(w)) {
      ...
      clr_bsy_brmask  (w) := GetNewBrMask(io.core.brupdate, mem_stq_incoming_e(w).bits.uop)
		////yh+begin
		//} .elsewhen (fired_bnd_incoming(w)) {
    //  clr_bsy_valid   (w) := mem_bdq_incoming_e(w).valid            &&
    //                        !IsKilledByBranch(io.core.brupdate, mem_bdq_incoming_e(w).bits.uop)
    //  clr_bsy_rob_idx (w) := mem_bdq_incoming_e(w).bits.uop.rob_idx
    //  clr_bsy_brmask  (w) := GetNewBrMask(io.core.brupdate, mem_bdq_incoming_e(w).bits.uop)

		//	printf("YH+ [%d] Clear busy bit bdq(%d) rob(%d) valid: %d killed: %d\n",
		//					io.core.tsc_reg, mem_bdq_incoming_e(w).bits.uop.bdq_idx,
		//					mem_bdq_incoming_e(w).bits.uop.rob_idx,
		//					mem_bdq_incoming_e(w).valid,
		//					!IsKilledByBranch(io.core.brupdate, mem_bdq_incoming_e(w).bits.uop))
		////yh+end
  ...
  for (w <- 0 until memWidth) {
    ...
    when (io.dmem.nack(w).valid)
    {
        ...
        .elsewhen (io.dmem.nack(w).bits.uop.uses_mcq)
      {
        ...
        mcq(io.dmem.nack(w).bits.uop.mcq_idx).bits.executed    := false.B

        printf("YH+ [%d] Received NACK mcq(%d)\n",
								io.core.tsc_reg, io.dmem.nack(w).bits.uop.mcq_idx)
      }
        .otherwise
      {
        assert(io.dmem.nack(w).bits.uop.uses_bdq)

        bdq(io.dmem.nack(w).bits.uop.bdq_idx).bits.executed    := false.B

        printf("YH+ [%d] Received NACK bdq(%d)\n",
								io.core.tsc_reg, io.dmem.nack(w).bits.uop.bdq_idx)
      }
      //yh+end
    }
    // Handle the response
    when (io.dmem.resp(w).valid)
    {
      ...
      {
        val bdq_resp_idx = io.dmem.resp(w).bits.uop.bdq_idx

        when (io.dmem.resp(w).bits.uop.is_bndstore)
        {
          bdq(bdq_resp_idx).bits.state        := b_done

          printf("YH+ [%d] Received Store RESP bdq(%d)\n",
                  io.core.tsc_reg, bdq_resp_idx)
        }
      }
      //yh+end
    }
      ...
      when (data_ready && live) {
		...
		//yh+begin
			.elsewhen (!dmem_resp_fired(w) && !wb_forward_valid(w) &&
									bdq_resp_val(w))
		{
			printf("YH+ [%d] Found bdq_resp_val! bdq_resp_idx: %d bnd_resp_val: %d\n",
							io.core.tsc_reg, bdq_bnd_idx, Mux(bdq_bnd_e.bits.state === b_bndResp, 1.U, 0.U))

      val bdq_uop     = bdq_bnd_e.bits.uop
      val live        = !IsKilledByBranch(io.core.brupdate, bdq_uop)
      val storegen = new freechips.rocketchip.rocket.StoreGen(
                                bdq_uop.mem_size, bnd_resp_paddr,
                                bnd_resp_data,
                                coreDataBytes)
      val loadgen  = new freechips.rocketchip.rocket.LoadGen(
                                bdq_uop.mem_size, bdq_uop.mem_signed,
                                bnd_resp_paddr,
                                storegen.data, false.B, coreDataBytes)

      io.core.exe(w).iresp.valid := live
      io.core.exe(w).iresp.bits.uop  := bdq_uop
      io.core.exe(w).iresp.bits.data := loadgen.data

      when (live) {
  			bdq_bnd_e.bits.executed := true.B
  			bdq_bnd_e.bits.state := b_done
      }
		}
		//yh+end
  }
  ...
  //yh+begin
  val mcq_load_resp_val = widthMap(w => io.dmem.resp(w).valid &&
		                                    io.dmem.resp(w).bits.uop.uses_mcq)
  val mcq_load_resp_idx = widthMap(w => io.dmem.resp(w).bits.uop.mcq_idx)
  val mcq_load_resp_data = widthMap(w => io.dmem.resp(w).bits.data)

  val bdq_load_resp_val = widthMap(w => io.dmem.resp(w).valid &&
                                        io.dmem.resp(w).bits.uop.uses_bdq &&
  																			io.dmem.resp(w).bits.uop.is_bndload)
  val bdq_load_resp_idx = widthMap(w => io.dmem.resp(w).bits.uop.bdq_idx)
  val bdq_load_resp_data = widthMap(w => io.dmem.resp(w).bits.data)
  val bdq_load_resp_bndstr = widthMap(w => io.dmem.resp(w).bits.uop.uopc === uopBNDSTR)
  val bdq_load_resp_bndclr = widthMap(w => io.dmem.resp(w).bits.uop.uopc === uopBNDCLR)
  val bdq_load_resp_bndsrch = widthMap(w => io.dmem.resp(w).bits.uop.uopc === uopBNDSRCH)

  for (w <- 0 until memWidth) {
    val mcq_idx = mcq_load_resp_idx(w)
    val mcq_uop = mcq(mcq_idx).bits.uop
    //val mcq_live = !IsKilledByBranch(io.core.brupdate, mcq_uop)

    when (mcq_load_resp_val(w)  &&
          //mcq_live              &&
          mcq(mcq_idx).valid    &&
          mcq(mcq_idx).bits.state === m_bndChk)
    {
      printf("YH+ [%d] mcq_load_resp_val(%d) is true\n",
              io.core.tsc_reg, w.U)

      val mcq_lbnd = Cat(false.B, mcq_load_resp_data(w)(31,0))
      val mcq_ubnd = Cat(false.B, (mcq_load_resp_data(w)(63,32) +
                                    mcq_load_resp_data(w)(31,0)))
      val c_bit = (mcq_load_resp_data(w)(31) & !mcq(mcq_idx).bits.addr(31))
      val mcq_addr = Cat(c_bit, mcq(mcq_idx).bits.addr(31,0))

      val bnd_chk = (mcq_lbnd <= mcq_addr && mcq_addr <= mcq_ubnd)
      val count = mcq(mcq_idx).bits.count

			printf("YH+ bnd_chk: %d c_bit: %d mcq_addr: %x, mcq_lbnd: %x, mcq_ubnd: %x\n",
							bnd_chk, c_bit, mcq_addr, mcq_lbnd, mcq_ubnd)
      ...
      when (bnd_chk)
      {
        ...
        printf("YH+ [%d] Passed bounds check! (bndchk) mcq(%d) addr: %x lbnd: %x ubnd: %x\n",
                io.core.tsc_reg, mcq_idx, mcq_addr, mcq_lbnd, mcq_ubnd)
      }
        .elsewhen (count < (hbt_num_way - 1.U))
      {
        mcq(mcq_idx).bits.executed    := false.B
        mcq(mcq_idx).bits.count       := count + 1.U

				printf("YH+ [%d] Increase counter (bndchk) (%d->%d) mcq(%d) addr: %x lbnd: %x ubnd: %x\n",
								io.core.tsc_reg, count, count+1.U, mcq_idx, mcq_addr, mcq_lbnd, mcq_ubnd)
      }
        .otherwise
      {
        mcq(mcq_idx).bits.state       := m_fail

        printf("YH+ [%d] Failed bounds check! (bndchk) mcq(%d) addr: %x lbnd: %x ubnd: %x\n",
                io.core.tsc_reg, mcq_idx, mcq_addr, mcq_lbnd, mcq_ubnd)
      }
    }

    val bdq_idx = bdq_load_resp_idx(w)
    val bdq_uop = bdq(bdq_idx).bits.uop
    //val bdq_live = !IsKilledByBranch(io.core.brupdate, bdq_uop)
    val bdq_data = bdq(bdq_idx).bits.data

    when (bdq_load_resp_val(w)  &&
          //bdq_live              &&
          bdq(bdq_idx).valid    &&
          bdq(bdq_idx).bits.state === b_occChk)
    {
      printf("YH+ [%d] bdq_load_resp_val(%d) is true\n",
              io.core.tsc_reg, w.U)

      //val occ_chk = Mux(bdq_load_resp_bndstr(w), true.B, //TODO
      val occ_chk = Mux(bdq_load_resp_bndstr(w), bdq_load_resp_data(w)(31,0) === 0.U, //TODO
													Mux(bdq_load_resp_bndsrch(w), bdq_load_resp_data(w) === bdq_data,
                          bdq_load_resp_data(w)(31,0) === bdq_data(31,0)))
      val count = bdq(bdq_idx).bits.count

      when (occ_chk)
      {
        when (bdq_load_resp_bndsrch(w))
        {
	        bdq(bdq_idx).bits.executed        := false.B
          bdq(bdq_idx).bits.state           := b_bndResp
          printf("YH+ [%d] Passed occupancy check (bndsrch)! bdq(%d) data: %x bnd: %x\n",
                  io.core.tsc_reg, bdq_idx, bdq_data, bdq_load_resp_data(w))
        }
          .otherwise
        {
          bdq(bdq_idx).bits.uop.mem_cmd     := rocket.M_XWR
          bdq(bdq_idx).bits.uop.is_bndload  := false.B
          bdq(bdq_idx).bits.uop.is_bndstore := true.B
          bdq(bdq_idx).bits.executed        := false.B
          bdq(bdq_idx).bits.state           := b_bndStr
					when (bdq_load_resp_bndstr(w))
					{
						printf("YH+ [%d] Passed occupancy check (bndstr)! bdq(%d) data: %x bnd: %x\n",
										io.core.tsc_reg, bdq_idx, bdq_data, bdq_load_resp_data(w))
					}
						.elsewhen (bdq_load_resp_bndclr(w))
					{
						printf("YH+ [%d] Passed occupancy check (bndclr)! bdq(%d) data: %x bnd: %x\n",
										io.core.tsc_reg, bdq_idx, bdq_data, bdq_load_resp_data(w))
					}
        }
      }
        .elsewhen (count < (hbt_num_way - 1.U))
      {
        ...
        when (bdq_load_resp_bndstr(w))
				{
					printf("YH+ [%d] Increase counter (bndstr) (%d->%d) bdq(%d) data: %x bnd: %x\n",
									io.core.tsc_reg, count, count+1.U, bdq_idx, bdq_data, bdq_load_resp_data(w))
				}
					.elsewhen(bdq_load_resp_bndsrch(w))
				{
					printf("YH+ [%d] Increase counter (bndsrch) (%d->%d) bdq(%d) data: %x bnd: %x\n",
									io.core.tsc_reg, count, count+1.U, bdq_idx, bdq_data, bdq_load_resp_data(w))
				}
					.elsewhen(bdq_load_resp_bndclr(w))
				{
					printf("YH+ [%d] Increase counter (bndclr) (%d->%d) bdq(%d) data: %x bnd: %x\n",
									io.core.tsc_reg, count, count+1.U, bdq_idx, bdq_data, bdq_load_resp_data(w))
				}
      }
        .otherwise
      {
	      ...
	      bdq(bdq_idx).bits.state           := b_fail

        when (bdq_load_resp_bndstr(w))
				{
          printf("YH+ [%d] Failed occupancy check! (bndstr) bdq(%d) data: %x bnd: %x\n",
                  io.core.tsc_reg, bdq_idx, bdq_data, bdq_load_resp_data(w))
				}
					.elsewhen(bdq_load_resp_bndsrch(w))
				{
          printf("YH+ [%d] Failed occupancy check! (bndsrch) bdq(%d) data: %x bnd: %x\n",
                  io.core.tsc_reg, bdq_idx, bdq_data, bdq_load_resp_data(w))
				}
					.elsewhen(bdq_load_resp_bndclr(w))
				{
          printf("YH+ [%d] Failed occupancy check! (bndclr) bdq(%d) data: %x bnd: %x\n",
                  io.core.tsc_reg, bdq_idx, bdq_data, bdq_load_resp_data(w))
				}
      } 
    }
  ...
  when (initWYFY)
  {
		printf("YH+ [%d] initWYFY num_signed_inst: %x num_unsigned_inst: %x\n",
						io.core.tsc_reg, io.core.wyfy_config.num_signed_inst, io.core.wyfy_config.num_unsigned_inst)
    ...
    .elsewhen (mcq_clear)
  {
    ...
    {
			//printf("YH+ [%d] Increment num_signed_inst: %x\n",
			//				io.core.tsc_reg, num_signed_inst + 1.U)

      num_signed_inst     := num_signed_inst + 1.U
    }
      .otherwise
    {
			//printf("YH+ [%d] Increment num_unsigned_inst: %x\n",
			//				io.core.tsc_reg, num_unsigned_inst + 1.U)
  ...
  when (initWYFY)
  {
    ...
    num_bndstr_fail       := io.core.wyfy_config.num_bndstr_fail
    num_bndclr_fail       := io.core.wyfy_config.num_bndclr_fail
  }
    .elsewhen (bdq_clear)
  {
    when (bdq(bdq_head).bits.uop.uopc === uopBNDSTR)
    {
      ...
      when (bdq(bdq_head).bits.state === b_fail)
      {
        num_bndstr_fail   := num_bndstr_fail + 1.U
      }
    }
      .elsewhen (bdq(bdq_head).bits.uop.uopc === uopBNDCLR)
    {
      ...
      when (bdq(bdq_head).bits.state === b_fail)
      {
        num_bndclr_fail   := num_bndclr_fail + 1.U
      }
    }
	...
	when (enableWYFY)
	{
		printf("YH+ [%d] mcq_head: %d mcq_tail: %d bdq_head: %d bdq_tail: %d ldq_head: %d ldq_tail: %d stq_head: %d stq_tail: %d\n",
						io.core.tsc_reg, mcq_head, mcq_tail, bdq_head, bdq_tail, ldq_head, ldq_tail, stq_head, stq_tail)
	}
  //yh+end
