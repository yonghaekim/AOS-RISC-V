...
class CSRFileIO(implicit p: Parameters) extends CoreBundle
  ...
  //yh+begin
  val wyfyConfig       	= UInt(xLen.W).asInput
  val hbt_base_addr     = UInt(xLen.W).asInput
  val hbt_num_way       = UInt(xLen.W).asInput
  val num_signed_inst   = UInt(xLen.W).asInput
  val num_unsigned_inst = UInt(xLen.W).asInput
  val num_bndstr        = UInt(xLen.W).asInput
  val num_bndclr        = UInt(xLen.W).asInput
  val num_bndsrch       = UInt(xLen.W).asInput
  val num_pacma        	= UInt(xLen.W).asInput
  val num_xpacm       	= UInt(xLen.W).asInput
  val num_bndstr_fail   = UInt(xLen.W).asInput
  val num_bndclr_fail   = UInt(xLen.W).asInput
  val num_bndchk_fail   = UInt(xLen.W).asInput
  val num_cache_hit    	= UInt(xLen.W).asInput
  val num_cache_miss   	= UInt(xLen.W).asInput
  //yh+end
}
...
class CSRFile(
  ...
  //yh+begin
  val regWyfyConfig       = Reg(UInt(xLen.W))
  val regHbtBaseAddr      = Reg(UInt(xLen.W))
  val regHbtNumWay        = Reg(UInt(xLen.W))
  val regNumSignedInst    = Reg(UInt(xLen.W))
  val regNumUnsignedInst  = Reg(UInt(xLen.W))
  val regNumBndStr        = Reg(UInt(xLen.W))
  val regNumBndClr        = Reg(UInt(xLen.W))
  val regNumBndSrch       = Reg(UInt(xLen.W))
  val regNumPacma         = Reg(UInt(xLen.W))
  val regNumXpacm         = Reg(UInt(xLen.W))
  val regNumBndStrFail    = Reg(UInt(xLen.W))
  val regNumBndClrFail    = Reg(UInt(xLen.W))
  val regNumBndChkFail    = Reg(UInt(xLen.W))
  val regNumCacheHit      = Reg(UInt(xLen.W))
  val regNumCacheMiss     = Reg(UInt(xLen.W))
 
  regWyfyConfig       := io.wyfyConfig.asUInt
  regHbtBaseAddr      := io.hbt_base_addr.asUInt
  regHbtNumWay        := io.hbt_num_way.asUInt
  regNumSignedInst    := io.num_signed_inst.asUInt
  regNumUnsignedInst  := io.num_unsigned_inst.asUInt
  regNumBndStr        := io.num_bndstr.asUInt
  regNumBndClr        := io.num_bndclr.asUInt
  regNumBndSrch       := io.num_bndsrch.asUInt
  regNumPacma         := io.num_pacma.asUInt
  regNumXpacm         := io.num_xpacm.asUInt
  regNumBndStrFail    := io.num_bndstr_fail.asUInt
  regNumBndClrFail    := io.num_bndclr_fail.asUInt
  regNumBndChkFail    := io.num_bndchk_fail.asUInt
  regNumCacheHit      := io.num_cache_hit.asUInt
  regNumCacheMiss     := io.num_cache_miss.asUInt

  val wyfyCSRId = 0x430
  read_mapping += wyfyCSRId -> regWyfyConfig
  val hbtBaseAddrCSRId = 0x431
  read_mapping += hbtBaseAddrCSRId -> regHbtBaseAddr
  val hbtNumWayCSRId = 0x432
  read_mapping += hbtNumWayCSRId -> regHbtNumWay
  val numSignedInstCSRId = 0x433
  read_mapping += numSignedInstCSRId -> regNumSignedInst
  val numUnsignedInstCSRId = 0x434
  read_mapping += numUnsignedInstCSRId -> regNumUnsignedInst
  val numBndStrCSRId = 0x435
  read_mapping += numBndStrCSRId -> regNumBndStr
  val numBndClrCSRId = 0x436
  read_mapping += numBndClrCSRId -> regNumBndClr
  val numBndSrchCSRId = 0x437
  read_mapping += numBndSrchCSRId -> regNumBndSrch
  val numPacmaCSRId = 0x438
  read_mapping += numPacmaCSRId -> regNumPacma
  val numXpacmCSRId = 0x439
  read_mapping += numXpacmCSRId -> regNumXpacm
  val numBndStrFailCSRId = 0x43a
  read_mapping += numBndStrFailCSRId -> regNumBndStrFail
  val numBndClrFailCSRId = 0x43b
  read_mapping += numBndClrFailCSRId -> regNumBndClrFail
  val numBndChkFailCSRId = 0x43c
  read_mapping += numBndChkFailCSRId -> regNumBndChkFail
  val numCacheHitCSRId = 0x43d
  read_mapping += numCacheHitCSRId -> regNumCacheHit
  val numCacheMissCSRId = 0x43e
  read_mapping += numCacheMissCSRId -> regNumCacheMiss
  //yh+end
