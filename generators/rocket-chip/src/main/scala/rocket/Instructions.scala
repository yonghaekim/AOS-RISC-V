...
object Instructions {
  ...
  //yh+begin
  def PACMA              = BitPat("b?????????????????100?????1110111")
  def XPACM              = BitPat("b?????????????????110?????1110111")
  def BNDSTR             = BitPat("b?????????????????000?????1110111")
  def BNDCLR             = BitPat("b?????????????????010?????1110111")
  def BNDSRCH            = BitPat("b?????????????????011?????1110111")
  //def BNDSTR             = BitPat("b?????????????????100000000100011")
  //def BNDCLR             = BitPat("b???????00000?????101000000100011")
  //def BNDSRCH            = BitPat("b?????????????????110?????0100011")
  //yh+end
...
object Causes {
  ...
  val store_page_fault = 0xf
	////yh+begin
	//val bnd_str_fault = 0x10
	//val bnd_chk_fault = 0x11
	//val bnd_clr_fault = 0x12
	////yh+end
